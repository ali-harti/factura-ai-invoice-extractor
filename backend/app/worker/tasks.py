...
def process_invoice(self, invoice_id: int):
    ...
    try:
        ...
        if invoice.status != "queued":
            logger.warning(
                f"Invoice {invoice_id} is in status '{invoice.status}', not 'queued'. Skipping."
            )
            return
        ...
        try:
            ...
            if invoice:
                ...
                invoice.error_message = f"Worker failed: {exc!s}"
                db.commit()
        ...
        raise self.retry(
            exc=exc, countdown=2**self.request.retries
        )  # exponential backoff
